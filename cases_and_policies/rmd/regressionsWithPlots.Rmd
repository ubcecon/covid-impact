...
```{r casedieff, eval = FALSE, cache=FALSE, warning=FALSE, results='asis'}
...
for (k in 1:length(infovars)) {
  ...
  piy[[(1+4*(k-1))]] <- policyreg(sdf2, yvar, pols, NULL,
              c(sprintf("lag(%s, %d)", infovars[[k]], L),
                tvars, xlist[[k]]),
              ilist[[k]], 0, L=L)
  # (2) adding mask wearing rate and log(Trump's vote)
  sdf <- subset(df, df$date>=as.Date("2020-03-07")) #+L)
  xlist <-list(c("z.mask","logvote"), c("z.mask","logvote"))
  piy[[(2+4*(k-1))]] <- policyreg(sdf, yvar, pols, NULL,
              c(sprintf("lag(%s, %d)", infovars[[k]], L),
                tvars, xlist[[k]]),
              ilist[[k]], 0, L=L)
  # (3) lagged behavior variables
  sdf <- subset(df, df$date>=as.Date("2020-02-22")) #+L)
  xlist <-list("","")
  piy[[(3+4*(k-1))]] <- policyreg(sdf, yvar, pols, NULL,
  ...
  # (4) (1)-(3)
   ...
   sdf2 <- sdf[which(sdf$state!='New York'),]
   xlist <-list(c("z.mask","logvote"), c("z.mask","logvote"))
   piy[[(4+4*(k-1))]] <- policyreg(sdf2, yvar, pols, NULL,
              c(sprintf("lag(%s, %d)", bvars, L+14),
                sprintf("lag(%s, %d)", infovars[[k]], L),
                tvars, xlist[[k]]),
              ilist[[k]], 0, L=L)
...
showhtmltables(NULL, NULL, piy);
# savetextables(NULL, NULL,  piy, prefix="deaths-robust")
　
...
for (k in 1:length(infovars)) { 
 ...
 showhtmltables(NULL, NULL,  piy);
# savetextables(NULL, NULL, piy, prefix="deaths-fe")
```
