...
```{r casetables, cache=TRUE, warning=FALSE, results='asis'}
...
```{r casedieff, cache=TRUE, warning=FALSE, results='asis'}
...
```{r deathtables, cache=TRUE, warning=FALSE, results='asis'}
infovars <- list(c("dlogdc", "logdc"),
                 c("dlogdc", "logdc","dlogdc.national", "logdc.national")) #,
df$zero <- 0
L <- 21
...
```{r deathtablesd, cache=TRUE, warning=FALSE, results='asis'}
df$zero <- 0
...
```{r dieff-deaths, cache=TRUE, results='asis', warning=FALSE}
for (use.national in c(TRUE,FALSE)) {
  sb <- ifelse(use.national, 2, 1)
  sy <- ifelse(use.national, 3, 1)
  pib <- lapply(regs$pib[[sb]][1:4], function(x) x$reg)
...
<!-- ```{r logdddc, cache=TRUE, warning=FALSE, results='asis'} -->
...
```{r fig, cache=TRUE,  fig_width=10, fig_height=6, warning=FALSE}
...
```{r polplots, cache=TRUE, fig_width=10, fig_height=6}
...
```{r poldeathsplots, cache=TRUE, fig_width=10, fig_height=6, warning=FALSE}
