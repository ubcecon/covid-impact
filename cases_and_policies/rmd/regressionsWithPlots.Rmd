...
wv <- c(svars,mvars.c,msvars.c,"month3") 
fmla <- as.formula(paste("dlogdc", "~",  paste(wv, collapse=" + "), sep=" "))
g1 <- predict(lm(fmla, data=df))
df$dlogdc.res <- df[,yv] - g1    
fig <- pfigure(df, "pmaskbus", stlab=-1,outcome="dlogdc.res",L=L.c) + ggtitle(TeX(relabel("dlogdc given pmaskbus"))) + ylab(TeX(relabel("residual of dlogdc"))) + 
  ylim(c(-1.8,0.5))  
p <- "pmaskbus"
...
wv <- c(svars,mvars.d,msvars.d,"month3") 
fmla <- as.formula(paste("dlogdd", "~",  paste(wv, collapse=" + "), sep=" "))
g1 <- predict(lm(fmla, data=df))
df$dlogdd.res <- df[,yv] - g1  
fig <- pfigure(df, "pmaskbus", stlab=-1,outcome="dlogdd.res",L=L.d) + ggtitle(TeX(relabel("dlogdd given pmaskbus"))) + ylab(TeX(relabel("residual of dlogdd")))+ 
  ylim(c(-0.9,0.5))  
p <- "pmaskbus"
