...
source(paste(rootdir,"cases_and_policies/rmd/rlassoPanel.R", sep="/"))
```
...
colnames(piy[[1]]$reg$response) <- "dlogdd"
showhtmltables(NULL, NULL,  piy);
#savetextables(NULL, NULL, piy[1:4], prefix="deaths-fe")
prefix <- "deaths-fe"
   ylbl <- colnames(piy[[1]]$reg$response)
    if (ylbl=="dlogdd") ylbl <- "Death Growth"
    if (ylbl=="dlogdc") ylbl <- "Case Growth"
...
L <- 14
df$dlogdc.L14 <- panellag(df$dlogdc,df$state,df$date, lag=L)
df$logdc.L14 <- panellag(df$logdc,df$state,df$date, lag=L)
df$dlogdd.L14 <- panellag(df$dlogdd,df$state,df$date, lag=L)
df$logdd.L14 <- panellag(df$logdd,df$state,df$date, lag=L)
df$pmaskbus.L14 <- panellag(df$pmaskbus,df$state,df$date, lag=L)
df$pk12.L14 <- panellag(df$pk12,df$state,df$date, lag=L)
df$pmovie.L14 <- panellag(df$pmovie,df$state,df$date, lag=L)
df$pshelter.L14 <- panellag(df$pshelter,df$state,df$date, lag=L)
df$prestaurant.L14 <- panellag(df$prestaurant,df$state,df$date, lag=L)
df$pnonessential.L14 <- panellag(df$pnonessential,df$state,df$date, lag=L)
df$pindex.L14 <- panellag(df$pindex,df$state,df$date, lag=L)
...
xvars <- c("logpop",
               ...
               "mask_percent", "logvote",
               "month3","month4","month5","month6","dlogdc.L14","logdc.L14")


pols.L14 <- c("pmaskbus.L14","pk12.L14","pmovie.L14","pshelter.L14","prestaurant.L14","pnonessential.L14")
...
DML2.for.PLM <- function(x, d, y, dreg, yreg, sdf, nfold=5) {
  ...
  for(b in 1:nfold){
    ...
    yfit <- yreg(x[I1,], y[I1]) # take a folot out
...
# dreg <- function(x,d){ rlasso(x, d, c=2, POST=FALSE) } #ML method = rlasso
# yreg <- function(x,y){ rlasso(x, y, c=2, POST=FALSE) } #ML method = rlasso
# DML2.lasso <- DML2.for.PLM(x, d, y, dreg, yreg, sdf)



x = as.matrix(cbind(sdf[,pols.L14[-1]],sdf[,xvars]))
d = sdf$pmaskbus.L14
y = sdf$dlogdc
...
set.seed(1)
DML2.lasso <- DML2.for.PLM(x, d, y, dreg, yreg, sdf, nfold = 5)
set.seed(1)
DML2.lasso <- DML2.for.PLM(x, d, y, dreg, yreg, sdf, nfold = 10)

dreg <- function(x,d){ randomForest(x, d) } #ML method=Forest
yreg <- function(x,y){ randomForest(x, y) } #ML method=Forest
set.seed(1)
DML2.RF <- DML2.for.PLM(x, d, y, dreg, yreg, sdf, nfold=5)
set.seed(1)
DML2.RF <- DML2.for.PLM(x, d, y, dreg, yreg, sdf, nfold=10)



x = as.matrix(cbind(sdf[,pols.L14[-4]],sdf[,xvars]))
d = sdf$pshelter.L14
y = sdf$dlogdc
...
x = as.matrix(cbind(sdf[,pols.L14[1:2]],sdf[,xvars]))
d = sdf$pindex.L14
y = sdf$dlogdc
...
xvars <- c("logpop",
               ...
               "Percent.at.risk.for.serious.illness.due.to.COVID","party_dum",
               "mask_percent", "logvote",
               "month3","month4","month5","month6","dlogdd.L14","logdd.L14")
