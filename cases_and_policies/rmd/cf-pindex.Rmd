...
if (!file.exists("cfindex_idx.Rda") || rerunsim) {
  ...
  save(cfshelterlist, shelterall, file="cfindex_idx.Rda")
}
...
figs <- nationalplots(shelterall, cfshelter, "no shutdown policies")
...
if (!file.exists("cfindex_deaths_idx.Rda") || rerunsim) {
  cfshelterlist <- lapply(unique(df$state), function(s) nopstatesim(s,simobs=simmodel,simcf=noshelter, data=sdf))
  shelterall <- rbindlist(lapply(cfshelterlist, getmeans))
  save(cfshelterlist, shelterall, file="cfindex_deaths_idx.Rda")
}
...
figs <- nationalplots(shelterall, cfshelter, "no shutdown policies", yvar="deaths")
