...
#   f <- cfplots(st, cfnop, data=df)
#
#   figdev(sprintf("%s/tex/tables_and_figures/%s-nop-dgrowth_idx.pdf",rootdir,st), width=5,height=3.75)
#   print(f$figdp)
#   dev.off()
#
#   figdev(sprintf("%s/tex/tables_and_figures/%s-nop-growth_idx.pdf",rootdir,st), width=5,height=3.75)
#   print(f$figd + ylim(c(-1,5)))
#   dev.off()
#
#   figdev(sprintf("%s/tex/tables_and_figures/%s-nop-cases_idx.pdf",rootdir,st), width=5,height=3.75)
#   print(f$figl)
#   dev.off()
#
#   figdev(sprintf("%s/tex/tables_and_figures/%s-nop-dcases_idx.pdf",rootdir,st), width=5,height=3.75)
#   print(f$figp)
#   dev.off()
# }
# ```
...
```{r masks-resids-fixed}
simmodel1 <- function(st, samplecoefs=FALSE, nsim=1)
  simmodel(st, samplecoefs=samplecoefs, nsim=nsim,
           shockgen=NULL)
maskmodel1 <- function(st, samplecoefs=FALSE, nsim=1)
  maskmodel(st, samplecoefs=samplecoefs, nsim=nsim,
            shockgen=NULL)

rerunsim <- FALSE
if (!file.exists("cfmask1_idx.Rda") || rerunsim) {
  cfmasklist1 <- lapply(unique(df$state),
                        function(s) nopstatesim(s,simobs=simmodel1,simcf=maskmodel1, data=sdf, nresid=1))
  maskall1 <- rbindlist(lapply(cfmasklist1, getmeans))
  save(cfmasklist1, maskall1, file="cfmask1_idx.Rda")
}
load("cfmask1_idx.Rda")
cfmask1 <- do.call(rbind, lapply(cfmasklist1, function(x) x$edf))

for (st in c("Washington","Illinois","Massachusetts")) {
  f <- cfplots(st, cfmask1, yvar="deaths", data=df)
  print(f$figp +  ggtitle(sprintf("Effect of mandating masks on April 1st in %s",st)))

}

figs1 <- nationalplots(maskall1, cfmask1, "mandating masks on April 1st\n")

for (f in figs1) print(f)
```
