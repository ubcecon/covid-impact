...
implies that 34 thousand lives could have been saved.  This finding is
...
a 90 percent confidence interval of $-7$\% to $50$\%.  These estimates
contribute to the ongoing efforts of evaluating various reopening
approaches.

The above findings are robust to changes to our regression specification, sample selection, methodology, and assumptions about delays between policy changes and changes in recorded cases. In all cases, we find that mask mandates, business closure policies, and stay-at-home orders all reduce growth rates by roughly 0.1. The impact of mask mandates are more precisely estimated than that of business closure policies or stay-at-home orders. This is largely due to the greater variation in the timing of mask mandates across states.
...
& {\icolor I^\star_{it} }& := &  I ({ t,  {\ycolor Y^*_{it}}, \icolor I^\star_{i,t-\ell}}),  \end{aligned}
\end{equation}
...
above. For examples, new cases are linked to growth rates via relation:
$$
\Delta C^{\star}_{it} = \exp\left (  \sum_{\bar t=1}^t  Y^\star_{i\bar t} \right) \text{ and } \Delta C_{it} = \exp
\left (  \sum_{\bar t=1}^t Y_{i\bar t}\right).
$$
...
$$
\Delta C^{\star}_{it}/\Delta C_{it} = \exp\left(  \sum_{\bar t=1}^t ( Y^\star_{i\bar t} -  Y_{i\bar t}) \right).
$$
...
shocks.\footnote{ If we'd want unconditional counterfactual,
we'd need to make assumptions about the evolution of stochastic shocks appearing in $Y_{it}$. See, e.g, previous
...
Figure \label{fig:WA-mask}, except for each state instead of just
